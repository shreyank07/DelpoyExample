// <copyright file="WriteableBitmapExtensions.cs" company="Prodigy Technovations Pvt. Ltd.">
//     Copyright (C)  2014
// </copyright>
...
#endif
{
    ...
    static partial class WriteableBitmapExtensions
    {
        ...
        public static void DrawPolyline(this WriteableBitmap bmp, Point[] points, Color color)
        {
            using (var context = bmp.GetBitmapContext())
            {
                // Use refs for faster access (really important!) speeds up a lot!
                var w = context.Width;
                var h = context.Height;
                var x1 = (long)(points[0].X);
                var y1 = (long)(points[0].Y);
                var col = ConvertColor(color);

                for (var i = 1; i < points.Length; i += 1)
                {
                    var x2 = (long)(points[i].X);
                    var y2 = (long)(points[i].Y);

                    DrawLongLine(context, w, h, x1, y1, x2, y2, col);
                    x1 = x2;
                    y1 = y2;
                }
            }
        ...
        /// <param name="y2">The y-coordinate of the end point.</param>
        /// <param name="color">The color for the line.</param>
        public static void DrawLongLine(BitmapContext context, int pixelWidth, long pixelHeight, long x1, long y1, long x2, long y2, int color)
        {
            var pixels = context.Pixels;
            ...
            { // x increases by +/- 1
                ...
                // Init steps and start
                long incy = (dy << PRECISION_SHIFT) / dx;

                long y1s = y1 << PRECISION_SHIFT;
                long y2s = y2 << PRECISION_SHIFT;
                long hs = pixelHeight << PRECISION_SHIFT;
                ...
                // Walk the line!
                long y = ys >> PRECISION_SHIFT;
                long previousY = y;
                long index = x1 + y * pixelWidth;
                int k = incy < 0 ? 1 - pixelWidth : 1 + pixelWidth;
            ...
            else
            {
                ...
                // Init steps and start
                long x1s = x1 << PRECISION_SHIFT;
                long x2s = x2 << PRECISION_SHIFT;
                long ws = pixelWidth << PRECISION_SHIFT;

                long incx = (dx << PRECISION_SHIFT) / dy;
                ...
                long index = x1s + ((y1 * pixelWidth) << PRECISION_SHIFT);
        ...
        public static void DrawText(this WriteableBitmap bitmap, FormattedText renderText, Point origin)
        {
            try
            {
                //Create bitmap for rendering Text on bitmap
                RenderTargetBitmap bmp = new RenderTargetBitmap((int)renderText.Width, (int)renderText.Height,
                    bitmap.DpiX, bitmap.DpiY, bitmap.Format);
                
                //Draw visual on the bitmap for rendering
                DrawingVisual visual = new DrawingVisual();
                using (var drawContent = visual.RenderOpen())
                {
                    drawContent.DrawText(renderText, new Point(0, 0));
                }
                //render the visual
                bmp.Render(visual);
                ...
                int widthInBytes = w * (bitmap.Format.BitsPerPixel + 7) / 8;
                int[] pixelData = new int[h * widthInBytes];
                bmp.CopyPixels(pixelData, widthInBytes, 0);
                //Copy the pixels from the rendered bitmap to the target bitmap at specified position
                bitmap.WritePixels(new Int32Rect((int)Math.Round(origin.X), (int)Math.Round(origin.Y), w, h), pixelData, widthInBytes, 0);
            }
            catch
            {
            }
        ...
        /// <param name="origin">The origin in the bitmap.</param>
        public static void DrawElement(this WriteableBitmap bitmap, FrameworkElement element, Point origin)
        {
            //Create bitmap for rendering Text on bitmap
            RenderTargetBitmap bmp = new RenderTargetBitmap((int)element.ActualWidth, (int)element.ActualHeight,
                bitmap.DpiX, bitmap.DpiY, bitmap.Format);
            ...
            bmp.CopyPixels(pixelData, widthInBytes, 0);
            //Copy the pixels from the rendered bitmap to the target bitmap at specified position
            bitmap.WritePixels(new Int32Rect((int)origin.X, (int)origin.Y, w, h), pixelData, widthInBytes, 0);
        }
